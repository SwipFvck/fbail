...
const makeMessagesSocket = (config) => {
   ...
   /*Inject Button node*/
  const inMsg = Utils_1.normalizeMessageContent(message) || null;
  const key = inMsg ? Utils_1.getContentType(inMsg) : null;
   function injectNativeNode(type, message) {
    switch (type) {
        ...
        case 'buttonsMessage': {
            ...
            if (btnName === 'review_and_pay') {
                return { tag: 'biz', attrs: { native_flow_name: 'order_details' } };
            }

            if (btnName === 'payment_info') {
                return { tag: 'biz', attrs: { native_flow_name: 'payment_info' } };
            }

            if (['mpm', 'cta_catalog', 'send_location', 'call_permission_request', 'wa_payment_transaction_details', 'automated_greeting_message_view_catalog'].includes(btnName)) {
                return {
                    tag: 'biz',
                    attrs: {},
                    content: [{
     ...
     if (!isNewsletter && key) {
     const nativeNode = injectNativeNode(key, message);
     const filteredNodes = Array.isArray(additionalNodes) ? filterNativeNode(additionalNodes) : [];
     const nodesToPush = [];
     if (filteredNodes) nodesToPush.push(...(Array.isArray(filteredNodes) ? filteredNodes : [filteredNodes]));
     if (nativeNode) nodesToPush.push(nativeNode);
...
const filterNativeNode = (nodeContent) => {
  if (Array.isArray(nodeContent)) {
    return nodeContent.filter((item) => {
      if (item.tag === 'biz' && (item.content && item.content.filter)) {
        return false;
      }
      return true;
    });
